...
playTable* pt=NULL;
chooseSoldierb::chooseSoldierb(QWidget *parent) : QMainWindow(parent)
{
    ...
    for(int j=0;j<10;j++){
        ...
        connect(icon[j],&QPushButton::clicked,[=](){
            ...
            if(icon[j]->isClicked){}
                ...
                for(int i=0;i<10;i++){
                    if(icon[i]->name==B.s[whichIsClicked].name)
                        icon[i]->notClicked();
                }
                B.s[whichIsClicked].setName(icon[j]->name);
                B.s[whichIsClicked].setSoldier(icon[j]->name);
                B.s[whichIsClicked].setIcon(icon[j]->common);
                icon[j]->beClicked();
...
void chooseSoldierb::paintEvent(QPaintEvent *event){
